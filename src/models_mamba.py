...
from cross_mamba_1d import CrossMamba1D   # 已有就跳过
...
class Block(nn.Module):
    ...
    def forward(
        self, hidden_states: Tensor, residual: Optional[Tensor] = None, x2: Optional[Tensor] = None, inference_params=None
    ):
        ...
        # 如果是 CrossMamba1D，就不要传 inference_params
        if isinstance(self.mixer, CrossMamba1D):
            hidden_states = self.mixer(hidden_states, x2)
        else:
