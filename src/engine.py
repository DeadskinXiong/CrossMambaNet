...
def pretrain_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        (x1, x2), _ = data  # data 是 ((x1,x2), label)
        x1, x2 = x1.to(device, non_blocking=True), x2.to(device, non_blocking=True)

        with amp_autocast():
            loss, _, _ = model(x1, mask_ratio=args.mask_ratio, x2=x2)
...
def evaluate(data_loader, model, device, if_stat=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        (x1, x2), target = batch  # 拆双视图
        x1, x2, target = x1.to(device), x2.to(device), target.to(device)
        target = target.to(device, non_blocking=True)

        # compute output
        with torch.cuda.amp.autocast():
            output = model(x1, x2=x2)       # 传 x2
    ...
    weighted_metrics = precision_recall_fscore_support(
        target_all, pred_all, average='weighted', zero_division=0
    )
    macro_metrics = precision_recall_fscore_support(
        target_all, pred_all, average='macro', zero_division=0
    )
    ...
    pre_per_class, rec_per_class, f1_per_class, support_per_class = (
        precision_recall_fscore_support(
            target_all, pred_all, average=None, zero_division=0
        )
    )
    ...
    print(
        '* Weighted: Pre {pre:.4f} Rec {rec:.4f} F1 {f1:.4f}'.format(
            pre=weighted_metrics[0],
            rec=weighted_metrics[1],
            f1=weighted_metrics[2]
        )
    )
    print(
        '* Macro:    Pre {pre:.4f} Rec {rec:.4f} F1 {f1:.4f}'.format(
            pre=macro_metrics[0],
            rec=macro_metrics[1],
            f1=macro_metrics[2]
        )
    ...
    test_state['weighted_pre'] = float(weighted_metrics[0])
    test_state['weighted_rec'] = float(weighted_metrics[1])
    test_state['weighted_f1'] = float(weighted_metrics[2])
    test_state['macro_pre'] = float(macro_metrics[0])
    test_state['macro_rec'] = float(macro_metrics[1])
    test_state['macro_f1'] = float(macro_metrics[2])
