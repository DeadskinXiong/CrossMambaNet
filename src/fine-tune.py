...
#20
    ...
    parser.add_argument('--model_kwopts', type=json.loads, default={})
    return parser
...
def main(args):
    ...
    #调试代码，证明fine-tune.py 里 build_dataset 已经换成 DualViewDataset，喂双视图
    '''
    sample = next(iter(data_loader_train))
    print('sample type :', type(sample))  # 应该是 <class 'list'>
    print('sample len  :', len(sample))  # 2
    x1, x2 = sample[0]  # 取出 (x1,x2) 元组
    print('x1/x2 shape :', x1.shape, x2.shape)  # 两个 (B,1,H,W)
    exit(0)
    '''
    ...
    if args.eval:
        ...
        print(f"Accuracy on the {len(dataset_val)} validation images: {test_stats['acc']:.4f}")
        print(f"Weighted-F1: {test_stats['weighted_f1']:.4f}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"Accuracy on the {len(dataset_val)} validation images: {test_stats['acc']:.4f}")
        print(f"Weighted-F1 on the validation set: {test_stats['weighted_f1']:.4f}")
        print(f"Macro-F1 on the validation set: {test_stats['macro_f1']:.4f}")

        max_accuracy = max(max_accuracy, test_stats["acc"])
        max_weighted_f1 = max(max_weighted_f1, test_stats["weighted_f1"])
        max_macro_f1 = max(max_macro_f1, test_stats["macro_f1"])

        print(f'Max Accuracy: {max_accuracy:.4f}')
        print(f'Max Weighted-F1: {max_weighted_f1:.4f}')
